...
public class ExceptionTranslator implements ProblemHandling {
   ...
   */
  @Override
  public ResponseEntity<Problem> handleThrowable(@Nonnull final Throwable throwable, @Nonnull final NativeWebRequest request) {
    ResponseStatus responseStatus = AnnotationUtils.findAnnotation(throwable.getClass(), ResponseStatus.class);
    if (responseStatus != null) {
      Problem problem = Problem.builder().withStatus(new HttpStatusAdapter(responseStatus.value()))
          .withTitle(responseStatus.reason().isEmpty() ? responseStatus.value().getReasonPhrase() : responseStatus.reason()).withDetail(throwable.getMessage()).build();
