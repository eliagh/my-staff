...
public class CompanyResource {
   ...
   * @return the ResponseEntity with status 201 (Created) and with body the new companyDTO, or with
   *         status 400 (Bad Request) if the company has already an ID
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the updated companyDTO, or with
   *         status 400 (Bad Request) if the companyDTO is not valid, or with status 500 (Internal
   *         Server Error) if the companyDTO couldn't be updated
   * @throws URISyntaxException if the Location URI syntax is incorrect
   */
  ...
  public ResponseEntity<CompanyDTO> updateCompany(@Valid @RequestBody CompanyDTO companyDTO) throws URISyntaxException {
    ...
    CompanyDTO result = companyService.save(companyDTO);
    return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, companyDTO.getId().toString())).body(result);
  }
   ...
   * @return the ResponseEntity with status 200 (OK) and with body the companyDTO, or with status
   *         404 (Not Found)
   */
