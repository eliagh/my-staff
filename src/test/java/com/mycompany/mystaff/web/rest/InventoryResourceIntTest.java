...
import com.mycompany.mystaff.repository.InventoryRepository;
import com.mycompany.mystaff.repository.search.InventorySearchRepository;
...
public class InventoryResourceIntTest {
  ...
  public void setup() {
    ...
    this.restInventoryMockMvc = MockMvcBuilders.standaloneSetup(inventoryResource).setCustomArgumentResolvers(pageableArgumentResolver).setControllerAdvice(exceptionTranslator)
  ...
  public static Inventory createEntity(EntityManager em) {
    Inventory inventory = new Inventory().quantity(DEFAULT_QUANTITY).inventoryDate(DEFAULT_INVENTORY_DATE);
    // Add required entity
  ...
  public void createInventory() throws Exception {
    ...
    restInventoryMockMvc.perform(post("/api/inventories").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(inventory)))
  ...
  public void getAllInventories() throws Exception {
    ...
    restInventoryMockMvc.perform(get("/api/inventories?sort=id,desc")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(inventory.getId().intValue()))).andExpect(jsonPath("$.[*].quantity").value(hasItem(DEFAULT_QUANTITY.doubleValue())))
  ...
  public void getInventory() throws Exception {
    ...
    // Get the inventory
    restInventoryMockMvc.perform(get("/api/inventories/{id}", inventory.getId())).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.id").value(inventory.getId().intValue())).andExpect(jsonPath("$.quantity").value(DEFAULT_QUANTITY.doubleValue()))
  ...
  public void updateInventory() throws Exception {
    ...
    updatedInventory.quantity(UPDATED_QUANTITY).inventoryDate(UPDATED_INVENTORY_DATE);

    restInventoryMockMvc.perform(put("/api/inventories").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(updatedInventory)))
  ...
  public void updateNonExistingInventory() throws Exception {
    ...
    restInventoryMockMvc.perform(put("/api/inventories").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(inventory)))
        .andExpect(status().isCreated());
  ...
  public void deleteInventory() throws Exception {
    ...
    // Get the inventory
    restInventoryMockMvc.perform(delete("/api/inventories/{id}", inventory.getId()).accept(TestUtil.APPLICATION_JSON_UTF8)).andExpect(status().isOk());
  ...
  public void searchInventory() throws Exception {
    ...
    restInventoryMockMvc.perform(get("/api/_search/inventories?query=id:" + inventory.getId())).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)).andExpect(jsonPath("$.[*].id").value(hasItem(inventory.getId().intValue())))
